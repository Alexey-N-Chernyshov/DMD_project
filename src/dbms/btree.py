...
class BTree(object):
    ...
    def _build_bulkloaded_branches(self, xxx_todo_changeme):
        (leaves, seps) = xxx_todo_changeme
        minimum = self.order // 2
        levels = [leaves]
...
class BPlusTree(BTree):
    ...
    def iterkeys(self):
        return map(operator.itemgetter(0), iter(self.iteritems()))

    def itervalues(self):
        return map(operator.itemgetter(1), iter(self.iteritems()))
